...
st.set_page_config(page_title="Trading Signals Improved", layout="wide", initial_sidebar_state="expanded")
...
def get_recent_candle_color(df, lookback=10):
    """Count recent green and red candles"""
    if len(df) < lookback:
        lookback = len(df)
...
def calculate_signal_and_levels(df, tf="15m"):
    ...
    if rsi_val > 55:
        score += 1; reasons.append("✅ RSI bullish (>55)")
    elif rsi_val < 45:
        score -= 1; reasons.append("❌ RSI bearish (<45)")
    if macd_h > 0:
        score += 1; reasons.append("✅ MACD histogram positive")
    else:
        score -= 1; reasons.append("❌ MACD histogram negative")
    if price <= bb_l * 1.005:
        score += 1; reasons.append("✅ Near lower Bollinger Band")
    ...
    if "BUY" in signal_type:
        ...
        tp2 = round(entry + risk * 2.2, 2)
        tp3 = round(max(r1, entry + risk * 3.0), 2)
        rr = "1 : 1.5+"
        ...
        tp1 = round(entry - risk * 1.5, 2)
        tp2 = round(entry - risk * 2.2, 2)
        tp3 = round(min(s1, entry - risk * 3.0), 2)
        rr = "1 : 1.5+"
    else:
    ...
    if "BUY" in signal_type:
        expected = max(3, min(7, int(score * 1.2)))
        candle_text = f"Expected {expected} green candles ahead"
    elif "SELL" in signal_type:
        expected = max(3, min(7, int(score * 1.2)))
        candle_text = f"Expected {expected} red candles ahead"
    else:
        candle_text = "Market is neutral"
    return {
        ...
        "recent_support": round(recent_low, 2), "recent_resistance": round(recent_high, 2),
        "rsi": round(rsi_val, 1), "ema9": round(ema9, 2), "ema21": round(ema21, 2),
        "atr": round(atr_val, 2), "last_price": round(price, 2),
        "candle_text": candle_text, "green_candles": green, "red_candles": red
    }
...
def build_chart(df, analysis, symbol_name, tf):
    ...
    fig.update_layout(title=f"{symbol_name} — {tf} | {analysis['signal']}", template="plotly_dark",
        height=480, margin=dict(l=10, r=10, t=50, b=10), xaxis_rangeslider_visible=False)
    return fig

st.markdown('<h1 class="main-header">📈 Trading Signals Improved</h1>', unsafe_allow_html=True)
st.caption("Gold • BTC • USDJPY • NAS100 | 30m + Better Signals + Candle Expectation")
...
if st.session_state.selected_symbol:
    ...
    if analysis:
        ...
        with setup_col:
            ...
            st.markdown(f"**Recent Low:** {analysis['recent_support']} &nbsp;&nbsp; **Recent High:** {analysis['recent_resistance']}")
            st.code(f"Entry: {analysis['entry']}\nSL: {analysis['sl']}\nTP1: {analysis['tp1']}  TP2: {analysis['tp2']}  TP3: {analysis['tp3']}")
        st.markdown("### 🕯️ Expected Candles")
        st.info(analysis['candle_text'])
        st.markdown("### 🧠 Why this signal?")
        for r in analysis['reasons']: st.write(r)
    else:
        st.error("Not enough data for this timeframe. Try a higher timeframe (like 1h or 4h).")

st.caption("Improved Version • Free Tier • Data via yfinance • Verify with your broker")
